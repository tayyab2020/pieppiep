...
class UserController extends Controller
{
    ...
    public function CreateNewQuotation()
    {
        ...
        if($user->can('create-new-quotation'))
        {
            ...
            return view('user.create_new_quotation1', compact('clients','employees','suppliers','last_event_id','quotation_ids','event_titles','current_appointments','products','customers','suppliers'));
        }
        else
        {
            return redirect()->route('user-login');
...
//        $commission_vat = $commission - $commission_vat;
    ...
    public function AcceptNewQuotation($id)
    {
        ...
        else
        {
            ...
            if($this->lang->lang == 'du')
            {
                $msg = "Beste " . $creator_name . ",<br><br>" . $user->name . " heeft offerte" . $invoice->quotation_invoice_number . "geaccepteerd.<br><br>Met vriendelijke groet,<br><br>Klantenservice<br><br> Pieppiep";
            }
            else
            {
                $msg = "Dear " . $creator_name . ",<br><br>Mr/Mrs " . $user->name . " has accepted your quotation QUO# " . $invoice->quotation_invoice_number . "<br><br>Kind regards,<br><br>Customer service<br><br> Pieppiep";
            }
    ...
    public function EditNewQuotation($id)
    {
        ...
        if($check)
        {
            ...
            if(\Route::currentRouteName() == 'view-new-quotation')
            {
                $invoice = new_quotations_data::leftjoin('new_quotations','new_quotations.id','=','new_quotations_data.quotation_id')->leftjoin('products','products.id','=','new_quotations_data.product_id')->where('new_quotations.id', $id)->where('new_quotations.creator_id', $user_id)->select('new_quotations.*','new_quotations_data.description as product_description','new_quotations_data.item_id','new_quotations_data.service_id','new_quotations.delivery_date as retailer_delivery_date','new_quotations.installation_date as retailer_installation_date','new_quotations.id as invoice_id','new_quotations_data.box_quantity','new_quotations_data.measure','new_quotations_data.max_width','new_quotations_data.order_number','new_quotations_data.discount','new_quotations_data.discount_option','new_quotations_data.labor_discount','new_quotations_data.total_discount','new_quotations_data.price_before_labor','new_quotations_data.labor_impact','new_quotations_data.model_impact_value','new_quotations_data.childsafe','new_quotations_data.childsafe_question','new_quotations_data.childsafe_answer','new_quotations_data.childsafe_x','new_quotations_data.childsafe_y','new_quotations_data.childsafe_diff','new_quotations_data.model_id','new_quotations_data.delivery_days','new_quotations_data.delivery_date','new_quotations_data.id','new_quotations_data.supplier_id','new_quotations_data.product_id','new_quotations_data.row_id','new_quotations_data.rate','new_quotations_data.basic_price','new_quotations_data.qty','new_quotations_data.amount','new_quotations_data.color','new_quotations_data.width','new_quotations_data.width_unit','new_quotations_data.height','new_quotations_data.height_unit','new_quotations_data.price_based_option','new_quotations_data.base_price','new_quotations_data.supplier_margin','new_quotations_data.retailer_margin','products.ladderband','products.ladderband_value','products.ladderband_price_impact','products.ladderband_impact_type')
                    ->with(['features' => function($query)
                    {
                        $query->leftjoin('features','features.id','=','new_quotations_features.feature_id')
                            /*->where('new_quotations_features.sub_feature',0)*/
            ...
            {
                $last_event_id = quotation_appointments::latest('id')->pluck('id')->first();
                $appointments = quotation_appointments::leftjoin('new_quotations','new_quotations.id','=','quotation_appointments.quotation_id')->leftjoin('customers_details as t1','t1.id','=','new_quotations.customer_details')->leftjoin('customers_details as t2','t2.id','=','quotation_appointments.retailer_client_id')->leftjoin('users as t3','t3.id','=','quotation_appointments.supplier_id')->leftjoin('users as t4','t4.id','=','quotation_appointments.employee_id')->where('quotation_appointments.quotation_id',$id)->select('quotation_appointments.*','t1.name as client_quotation_fname','t1.family_name as client_quotation_lname','t2.name as client_fname','t2.family_name as client_lname','t3.company_name','t4.name as employee_fname','t4.family_name as employee_lname')->get();
                $other_appointments = quotation_appointments::leftjoin('new_quotations','new_quotations.id','=','quotation_appointments.quotation_id')
                ...
                ->leftjoin('users as t3','t3.id','=','quotation_appointments.supplier_id')
                ->leftjoin('users as t4','t4.id','=','quotation_appointments.employee_id')
                ->where(function($query1) use($id) {
                    $query1->where('quotation_appointments.quotation_id','!=',$id)->orWhere('quotation_appointments.quotation_id',NULL);
                })
...
//                        $row->eventDurationEditable = false;
                ...
                $event_titles = planning_titles::where('user_id',$user_id)->get();
                $quotation_ids = new_quotations::leftjoin('customers_details','customers_details.id','=','new_quotations.customer_details')->where('new_quotations.creator_id',$user_id)->where('new_quotations.id','!=',$id)->select('new_quotations.id','new_quotations.quotation_invoice_number','customers_details.name','customers_details.family_name')->get();
                $clients = customers_details::where('retailer_id',$user_id)->get();
                $suppliers = User::leftjoin('retailers_requests', function($join) use($user_id){
                    $join->on('users.id', '=', 'retailers_requests.supplier_id');
                    $join->where('retailers_requests.retailer_id',$user_id);
    ...
    public function Plannings()
    {
        ...
        $employees = User::where('role_id',2)->where('main_id',$user_id)->where('id','!=',$user->id)->get();
    ...
    public function StoreNewQuotation(Request $request)
    {
            ...
            if($request->is_invoice)
            {
                ...
                else
                {
                    if(!$request->negative_invoice_id)
                    {
                        ...
                        $org_invoice_data->subtotal = str_replace(',', '.',str_replace('.', '',$request->total_amount));
                        $org_invoice_data->grand_total = str_replace(',', '.',str_replace('.', '',$request->total_amount));
                        $org_invoice_data->price_before_labor_total = str_replace(',', '.',str_replace('.', '',$request->price_before_labor_total));
                        ...
                        if($form_type == 2)
                        {
                            ...
                            $org_invoice_data->labor_cost_total = str_replace(',', '.',str_replace('.', '',$request->labor_cost_total));
                        }
                        else
                        {
        ...
        else
        {
            ...
            $invoice->subtotal = str_replace(',', '.',str_replace('.', '',$request->total_amount));
            $invoice->grand_total = str_replace(',', '.',str_replace('.', '',$request->total_amount));
            $invoice->price_before_labor_total = str_replace(',', '.',str_replace('.', '',$request->price_before_labor_total));
            ...
            if($form_type == 2)
            {
                ...
                $invoice->labor_cost_total = str_replace(',', '.',str_replace('.', '',$request->labor_cost_total));
            }
            else
            {
            ...
            $invoice->net_amount = str_replace(',', '.',str_replace('.', '',$request->net_amount));
            $invoice->tax_amount = str_replace(',', '.',str_replace('.', '',$request->tax_amount));
            $invoice->save();
    ...
    public function RetailerMarkDelivered($id)
    {
        ...
        if($data)
        {
            ...
            if($this->lang->lang == 'du')
            {
                $msg = "Beste ".$client_name.",<br><br> Status update: goederen zijn afgeleverd. <b>".$retailer_company."Conform offerte <b>" . $quotation_invoice_number . "</b>.<br><br>Met vriendelijke groet,<br><br>Klantenservice<br><br>$retailer_name";
                $sub_mail = "Status gewijzigd naar bezorgd";
            }
            else
            {
                $msg = "Recent activity: Hi ".$client_name.", quotation has been delivered by retailer <b>".$retailer_company."</b><br> Quotation No: <b>" . $quotation_invoice_number . "</b>.<br><br>Kind regards,<br><br>Customer service<br><br> $retailer_name";
                $sub_mail = "Quotation marked as delivered by retailer!";
            }

            \Mail::send(array(), array(), function ($message) use ($msg, $sub_mail, $client_email, $retailer_company, $client_name, $quotation_invoice_number, $retailer_email, $retailer_name) {
