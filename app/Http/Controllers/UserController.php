...
class UserController extends Controller
{
    ...
    public function CustomerQuotations($id = '')
    {
        ...
        if($user->can('create-new-quotation'))
        {
            if($user_role == 2)
            {
                $new_invoices = new_quotations::leftjoin('customers_details', 'customers_details.id', '=', 'new_quotations.customer_details')->where('new_quotations.creator_id', $user_id)->where('new_quotations.status','!=',3)->orderBy('new_quotations.created_at', 'desc')->select('new_quotations.*', 'new_quotations.id as invoice_id', 'new_quotations.created_at as invoice_date', 'customers_details.name', 'customers_details.family_name')->with('orders')->with('invoices')->get();
            }
            else
            {
                $new_invoices = new_quotations::leftjoin('new_orders', 'new_orders.quotation_id', '=', 'new_quotations.id')->leftjoin('customers_details', 'customers_details.id', '=', 'new_quotations.customer_details')->where('new_orders.supplier_id', $user_id)->where('new_quotations.finished',1)->orderBy('new_quotations.created_at', 'desc')->select('new_quotations.*', 'new_quotations.id as invoice_id', 'new_orders.order_sent', 'new_orders.id as data_id', 'new_quotations.created_at as invoice_date', 'new_orders.order_number','new_orders.approved as data_approved','new_orders.processing as data_processing','new_orders.delivered as data_delivered', 'customers_details.name', 'customers_details.family_name')->with('invoices')->get();
    ...
    public function AcceptQuotation($request,$user_id)
    {
        ...
        new_quotations::where('id', $request->invoice_id)->update(['ask_customization' => 0, 'accepted' => 1, 'accept_date' => $now, 'delivery_date' => $delivery_date]);
        ...
        \Mail::send(array(), array(), function ($message) use ($msg, $retailer_email, $user_name, $invoice, $user) {
            $message->to($retailer_email)
                ->from('info@vloerofferte.nl')
